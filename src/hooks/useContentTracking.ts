...
export function useContentDailyLogs(startDate?: string, endDate?: string) {
  return useQuery({
    queryKey: ['content-daily-logs', startDate, endDate],
    queryFn: async () => {
...
export function useAutoGenerateRequiredItems() {
  ...
  return useMutation({
    mutationFn: async ({ date }: { date: string }) => {
      ...
      const template = getDailyTemplate(new Date(date + 'T12:00:00'));
