...
import { useTrafegoPagoClientes, useTrafegoPagoAllRegistros, TrafegoPagoCliente } from '@/hooks/useTrafegoPago';
...
const META_KEY = 'w3_trafego_mrr_meta';
...
export default function TrafegoPagoDashboard() {
  ...
  const { data: allRegs, dataUpdatedAt } = useTrafegoPagoAllRegistros();
  ...
  const mrrPrev = regsPrev.reduce((s, r) => {
    const c = clienteMap[r.cliente_id];
    return c?.status === 'Ativo' || c?.status === 'Cancelado' || c?.status === 'Pausado' ? s + Number(r.valor_pago) : s;
  }, 0);
  ...
  const totalInvestido = regsAtual.reduce((s, r) => s + Number(r.investimento_gerenciado), 0);
  const totalRecebido = regsAtual.reduce((s, r) => s + Number(r.valor_pago), 0);
  const churnRate = mrrPrev > 0 ? (churn / mrrPrev) * 100 : 0;

  // NRR: clients that existed prev month, what they pay now vs then
  const nrr = useMemo(() => {
  ...
  const prevNovo = useMemo(() => {
    ...
    return prevNovoClientes.reduce((s, c) => s + Number(c.valor_mrr), 0);
  }, [clientes, prevMonth]);
  ...
  const monthlyData = useMemo(() => {
    ...
    allRegs?.forEach(r => {
      const c = clienteMap[r.cliente_id];
      if (c) {
        map[r.mes_ano] = (map[r.mes_ano] || 0) + Number(r.valor_pago);
      }
    });
    return map;
  }, [allRegs, clienteMap]);
  ...
  const statusGroups = useMemo(() => {
    ...
    clientes?.forEach(c => {
      if (groups[c.status]) {
        ...
        groups[c.status].mrr += Number(c.valor_mrr);
      }
    });
    // Count Atrasado from regs
    const atrasados = new Set(regsAtual.filter(r => r.status_pagamento === 'Atrasado').map(r => r.cliente_id));
    groups['Atrasado'] = {
      count: atrasados.size,
      mrr: Array.from(atrasados).reduce((s, id) => s + Number(clienteMap[id]?.valor_mrr || 0), 0),
    };
    return [
  ...
  const gestorPerf: GestorRow[] = useMemo(() => {
    const map: Record<string, { clientes: Set<string>; investimento: number; receita: number }> = {};
    regsAtual.forEach(r => {
      const c = clienteMap[r.cliente_id];
      const gId = c?.gestor_user_id || 'sem-gestor';
      if (!map[gId]) map[gId] = { clientes: new Set(), investimento: 0, receita: 0 };
    ...
    return Object.entries(map).map(([id, d]) => ({
      id, nome: gestorMap[id] || 'Sem gestor', clientes: d.clientes.size, investimento: d.investimento, receita: d.receita,
  ...
  return (
    <AppLayout>
      <PageHeader title="Visão Geral — Tráfego Pago">
        <MonthYearSelector value={selectedMonth} onChange={setSelectedMonth} />
        <div className="flex items-center gap-2">
          <Switch id="compare-tp" checked={compareEnabled} onCheckedChange={setCompareEnabled} />
          <Label htmlFor="compare-tp" className="text-xs whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.5)' }}>Comparar</Label>
        </div>
      ...
      {/* Tráfego Pago exclusive KPIs */}
      <KPIRow items={[
        { label: 'Total Investido em Tráfego', value: formatCurrency(totalInvestido), tooltip: 'Soma do investimento em mídia de todos os clientes no período' },
        { label: 'Total Recebido em Gestão', value: formatCurrency(totalRecebido), tooltip: 'Soma do que seus clientes pagaram pela gestão no período', color: '#F97316' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <div className="space-y-4">
          ...
          <GestorPerfCard rows={gestorPerf} type="trafego" />
        </div>
        <div className="space-y-4">
